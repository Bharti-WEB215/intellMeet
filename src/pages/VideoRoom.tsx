...
export const VideoRoom: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 w-full text-[var(--theme-text)] min-h-[calc(100vh-140px)] animate-fadeIn">
        ...
        <GlassCard className="border-[var(--theme-border)] p-4 space-y-4 h-full flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[var(--theme-divider)]">
              <h3 className="text-xs font-bold text-slate-355 flex items-center gap-1.5">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2.5">
                  <div className="relative">
                    <img className="w-7 h-7 rounded-full object-cover" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=80" alt="Local User" />
                  </div>
                  <div>
                ...
                <div key={p.socketId} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2.5">
                    <div className="relative">
                      ...
                      {p.isSpeaking && (
                        <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-accent border-2 border-[var(--theme-surface)]" />
                      )}
                    </div>
                    <div>
      ...
      <div className="flex-1 flex flex-col space-y-4">
        ...
        <AnimatePresence>
          {decisionAlert && (
            <motion.div
              ...
              exit={{ y: -30, opacity: 0 }}
              className="bg-accent/20 border border-accent/40 rounded-xl p-3.5 text-xs text-accent flex items-center justify-between shadow-lg"
            >
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 min-h-[350px]">
          {/* Local Feed */}
          <div className="relative rounded-3xl overflow-hidden border border-[var(--theme-border)] bg-[var(--theme-surface)] p-0.5 shadow-xl transition-all duration-300">
            <div className="absolute inset-0 bg-slate-955 overflow-hidden flex items-center justify-center">
              {isVideoOff ? (
                <div className="flex flex-col items-center space-y-3">
                  <img className="w-20 h-20 rounded-full object-cover border-4 border-[var(--theme-surface)] shadow-2xl" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=80" alt="Self" />
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center z-10">
                <div className="rounded-lg bg-[var(--theme-input-bg)] backdrop-blur-md border border-[var(--theme-border)] px-2.5 py-1 text-[10px] font-bold">
          ...
          {peers.map(p => (
            <div 
              key={p.socketId}
              className={`relative rounded-3xl overflow-hidden border bg-[var(--theme-surface)] p-0.5 shadow-xl transition-all duration-300 ${
                p.isSpeaking ? 'ring-2 ring-primary border-primary shadow-primary/20 scale-[1.01]' : 'border-[var(--theme-border)]'
              }`}
            >
              <div className="absolute inset-0 bg-slate-955 overflow-hidden flex items-center justify-center">
                <video 
          ...
          {/* Empty Lane */}
          {peers.length === 0 && (
            <div className="border border-dashed border-[var(--theme-divider)] rounded-3xl flex items-center justify-center py-12 text-center text-[10px] text-[var(--theme-text-muted)] font-mono uppercase tracking-wider">
        ...
        <div className="flex justify-center items-center py-2 relative z-25">
          <div className="glass-panel-light rounded-2xl px-6 py-3.5 flex items-center justify-between gap-6 shadow-2xl border-[var(--theme-border)]">
            {/* Toggle Mic */}
            <button 
              onClick={toggleMute}
            ...
            <button 
              ...
              className="p-3 rounded-xl bg-red-650 hover:bg-red-700 text-white border border-red-600/35 shadow-lg shadow-red-700/25 transition-all cursor-pointer"
          ...
          <div className="space-y-4 flex-1 flex flex-col">
            ...
            <div className="flex bg-[var(--theme-bg)] p-0.5 rounded-lg border border-[var(--theme-divider)] w-full">
              <button 
                ...
                className={`flex-1 py-1.5 text-[10px] font-bold rounded-md uppercase text-center transition-colors cursor-pointer ${
                ...
                {activeRightTab === 'transcript' ? (
                  ...
                  >
                    {liveTranscript.map((t, idx) => (
                      <div key={idx} className="space-y-1 bg-[var(--theme-surface-alt)] border border-[var(--theme-divider)] p-2.5 rounded-xl">
                  ...
                  >
                    {chatMessages.map((msg, idx) => (
                      <div key={idx} className="space-y-1 bg-[var(--theme-input-bg)] border border-[var(--theme-divider)] p-2 rounded-xl">
                        ...
                        <p className="text-xs text-slate-3-00 font-sans leading-normal">{msg.text}</p>
                      </div>
                    ))}
                  </motion.div>
                )}
            ...
            {activeRightTab === 'chat' ? (
              <form onSubmit={handleSendChatMessage} className="flex gap-1.5 items-center">
                <input 
                  ...
                  className="flex-1 bg-[var(--theme-bg)] border border-[var(--theme-border)] rounded-lg px-2.5 py-1.5 text-xs text-[var(--theme-text)] focus:outline-none"
                />
                <button type="submit" className="p-1.5 bg-primary rounded-lg text-white cursor-pointer">
              ...
              <div className="bg-primary/20 border border-primary/30 p-3 rounded-xl">
                ...
                <p className="text-[11px] text-[var(--theme-text)] leading-normal">
